...
#define GNOME_SYSTEM_MONITOR "/usr/bin/mate-system-monitor &"
#define SHOW_DESKTOP "/usr/bin/show_desktop"
...
static void
panel_context_menu_task_manager (GtkWidget *w,
                             gpointer data)
{
       char command[256];
       sprintf(command,"%s",GNOME_SYSTEM_MONITOR);
       system(command);
       return 0;
}
...
{
       char command[256];
       sprintf(command,"%s",SHOW_DESKTOP);
       system(command);
       return 0;
}
...
#endif
       menuitem = gtk_image_menu_item_new_with_mnemonic (_("_Task Manager"));
       image = gtk_image_new_from_stock (GTK_STOCK_PROPERTIES, GTK_ICON_SIZE_MENU);
       gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (menuitem), image);
       gtk_widget_show (menuitem);
       gtk_menu_shell_append (GTK_MENU_SHELL (retval), menuitem);
       g_signal_connect (menuitem, "activate",
                         G_CALLBACK (panel_context_menu_task_manager), NULL);

       menuitem = gtk_image_menu_item_new_with_mnemonic (_("_Show desktop"));
       image = gtk_image_new_from_stock (GTK_STOCK_HOME, GTK_ICON_SIZE_MENU);
       gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (menuitem), image);
       gtk_widget_show (menuitem);
       gtk_menu_shell_append (GTK_MENU_SHELL (retval), menuitem);
       g_signal_connect (menuitem, "activate",
                         G_CALLBACK (panel_context_menu_show_desktop), NULL);
